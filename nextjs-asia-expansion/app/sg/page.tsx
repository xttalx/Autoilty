...
import { ListingSchemaScript } from '@/lib/schema/listing-schema';

export default function SingaporePage() {
  const { t } = useTranslation();
  const country: CountryCode = 'SG';
  ...
  useEffect(() => {
    const loadDeals = async () => {
      ...
      try {
        const dealsData = await fetchDeals('SG', 8);
        setDeals(dealsData);
      } catch (error) {
  ...
  const handleFiltersChange = (newFilters: SearchFiltersType) => {
    setFilters({ ...newFilters, country: 'SG', page: 1 });
  };
  ...
  return (
    <>
      {/* Schema markup for all listings */}
      {listings.map((listing) => (
        <ListingSchemaScript key={listing.id} listing={listing} country={country} />
      ))}
      ...
      <div className="min-h-screen bg-gray-50">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center">
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4">
                Find Your Perfect Car in Singapore
              </h1>
        ...
        {!loadingDeals && deals.length > 0 && (
          <section className="py-6 sm:py-8 lg:py-10 bg-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <DealCarousel deals={deals} country={country} title="🔥 Best Deals in Singapore" />
            </div>
          </section>
        )}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                  {listings.map((listing) => (
                    <DirectoryListing key={listing.id} listing={listing} country={country} />
                  ))}
                </div>
